...
if($size>0) {
  ...
  $content = preg_replace('/ftpUser1:\s*"(.*)"/','ftpUser1: "'.$users[0].'"',$content);
  $content = preg_replace('/ftpPass1:\s*"(.*)"/','ftpPass1: "'.$passs[0].'"',$content);

  if($size == 2) {
    $content = preg_replace('/ftpUser2:\s*"(.*)"/','ftpUser2: "'.$users[1].'"',$content);
    $content = preg_replace('/ftpPass2:\s*"(.*)"/','ftpPass2: "'.$passs[1].'"',$content);
  }
